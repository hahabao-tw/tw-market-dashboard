...
def update_options():
    ...
    # {date: {month: {strike: {'c':oi,'p':oi}}}}
    book = {}
    for r in rows[1:]:
        if len(r) <= i_oi:
            continue
        month = r[i_month].strip()
        if not (month.isdigit() and len(month) == 6):   # 只留月選,排除週選
            continue
        ...
        d = r[i_date].strip().replace("/", "-")
        book.setdefault(d, {}).setdefault(month, {}).setdefault(strike, {"c": 0, "p": 0})
        book[d][month][strike][cp] = oi
    ...
    months = []
    for month in sorted(book[latest].keys()):
        chain = book[latest][month]
...
def update_margin():
    ...
    # 限制單次最多補 25 天,避免對證交所請求過多
    targets = targets[-25:]
    changed = False
    for d in targets:
        res = fetch_margin_day(d)
        time.sleep(3)  # 證交所對頻繁請求很敏感,務必放慢
        if res is None:
            continue
        bal_k, buy_k = res          # 仟元
        amt = turnover[d]            # 元
        rows.append({
            "date": d,
            "balance": round(bal_k / 1e5, 1),          # 億元
            "turnover": round(amt / 1e8, 1),           # 億元
            "ratio": round(buy_k * 1000 / amt * 100, 2) if amt else None,  # 融資買進佔成交金額 %
        })
        changed = True
